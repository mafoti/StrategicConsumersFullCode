...
int double_controller::init(OBJECT *parent){
	...
	//pNextP = 	gl_get_double_by_name(pMarket, "next.price");
...
TIMESTAMP double_controller::presync(TIMESTAMP t0, TIMESTAMP t1){
	//printf("double controller presync\n");
	//printf("\n\n\npresync thermostatmode = %d deadband=%f *deadband_ptr=%f\n ", (int)thermostat_mode, deadband, *deadband_ptr);
	char timet0[128];
	gl_printtime(t0, timet0, 127);
	char timet1[128];
	gl_printtime(t1, timet1, 127);
	char timenext[128];
	gl_printtime(next_run, timenext, 127);
	//printf("to=%s, t1=%s, next_run=%s \n", timet0, timet1, timenext);
	...
	//printf("presync cool_set_base=%f heat_set_base=%f\n ", cool_set_base, heat_set_base);
	//printf("presync thermostatmode = %d cool_setpoint=%f heat_setpoint=%f\n ", (int)thermostat_mode, cool_setpoint, heat_setpoint);
	//printf("presync cool_temp_min = %f cool_temp_max=%f heat_temp_min=%f heat_temp_max=%f temperature=%f\n ", cool_temp_min, cool_temp_max, heat_temp_min, heat_temp_max, temperature);
	return TS_NEVER;
}

TIMESTAMP double_controller::sync(TIMESTAMP t0, TIMESTAMP t1){
	//printf("inside sync of double controller\n");
	//printf("sync \n thermostatmode = %d deadband=%f *deadband_ptr=%f\n ", (int)thermostat_mode, deadband, *deadband_ptr);
	//printf("double controller sync\n");
	double bid = -1.0;
	double demand = 0.0;
	...
	// clip setpoints and bids default resolve_mode=NONE
	switch(resolve_mode){
		case RM_DEADBAND:
	...
	if(bid_mode == BM_ON){
		...
		if (enthousiasm > 0.18 && counter <= 3){
			bid_quant = 0.0;
		}
		
		if (bid_quant!=0)
		{
			//printf("submit bid with bid_quant=%f, bid_price=%f, bid_id=%d \n", bid_quant, bid_price, (int)bid_id);
			if(state_ptr != 0){
...
TIMESTAMP double_controller::postsync(TIMESTAMP t0, TIMESTAMP t1){
	if(t0 < next_run){
		//printf("postsync return next_run \n");
		return next_run;
		//return TS_NEVER;
	}
	//printf("double controller postsync\n");
	...
	if(*pMarketID != lastmkt_id){
		...
		avg_price = 0.0;
		//printf("postsync *pMarketID != lastmkt_id *pAvg=%f, *pStd=%f, cool_set_base=%f, heat_set_base=%f, cool_bid=%f, heat_bid=%f \n", *pAvg, *pStd, cool_set_base, heat_set_base, cool_bid, heat_bid);
		if(*pAvg == 0.0 || *pStd == 0.0 || cool_set_base == 0.0 || heat_set_base == 0.0){
			//printf("postsync not enough input data\n");
			//return next_run; // not enough input data
			return TS_NEVER;
		}

		price = market->current_frame.clearing_price;
		avg_price = *pAvg;
		stdev_price = *pStd;
		//printf("clearing_price=%f, avg_price=%f, bid_price=%f last_mode=%d\n", price, avg_price, bid_price, (int)last_mode);
		if (price > *pAvg){ // high prices un-cool and un-heat
		...
		if (bid_mode == BM_ON && price > bid_price){ // only if we care about 'beating the market'
			...
			if(cool_bid > 0.0 || bid_mode == BM_OFF){
				...
				double cool_offset = (price - *pAvg) * fabs(cool_bound - cool_set_base) / (ramp * *pStd);
			...
			if(heat_bid > 0.0 || bid_mode == BM_OFF){
				...
				double heat_offset = (price - *pAvg) * fabs(heat_bound - heat_set_base) / (ramp * *pStd);
	...
	//if (testcase.compare(namebuf) == 0){
		...
		trainer.setNextAuctionTime(timenext_run);
		enthousiasm = trainer.getresult();
		if (enthousiasm > 0.18){
			if (counter < 3){
