...
TIMESTAMP generator_controller::sync(TIMESTAMP t0, TIMESTAMP t1)
{
				...
				for (index = 0; index<bid_curve_values[latency_write_section].number_bid_curve_sections; index++)
				{
					...
					if (gen_state != GEN_OFF)	//Generator is not off, so must be starting/started
					{
						...
						if (gl_object_isa(parent2, "solar", "generators"))
						{
							...
							if (enthousiasm > 0 && enthousiasm > contr_price){
								contr_price = contr_price + 0.01;
								*bid_price = contr_price;
							}
							else{
								*bid_price = init_bid_price;
								contr_price = init_bid_price;
							}
							bidID_val = auction_object->submit(obj, ((*complex_power).Re()) / 1000, contr_price, -1, BS_ON);

							//bidID_val=auction_object->submit(obj,((*complex_power).Re())/1000,bid_curve_values[latency_write_section].Curve_Info[index].price,-1,BS_ON);
					...
					else	//Must be off then
					{
						...
						if (gl_object_isa(parent2, "solar", "generators"))
						{
							...
							bidID_val = auction_object->submit(obj, (((*complex_power).Im()) / 1000), contr_price, -1, BS_OFF);
							//bidID_val=auction_object->submit(obj,(((*complex_power).Re())/1000),bid_curve_values[latency_write_section].Curve_Info[index].price,-1,BS_OFF);
				...
				else	//Only 1
				{
					...
					if (gen_state == GEN_OFF)
					{
						...
						if (gl_object_isa(parent2, "solar", "generators"))
						{
							...
							bidID_val = auction_object->submit(obj, (-(*complex_power).Re()) / 1000, contr_price, -1, BS_ON);
							//bidID_val=auction_object->submit(obj,(-(*complex_power).Re())/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_ON);
					...
					{	//Must be on or starting, so building is "off"
						...
						if (gl_object_isa(parent2, "solar", "generators"))
						{
							...
							bidID_val = auction_object->submit(obj, (-(*complex_power).Re()) / 1000, contr_price, -1, BS_OFF);
							//bidID_val=auction_object->submit(obj,(-(*complex_power).Re())/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_OFF);
	...
	if (update_output == true)	//Calculate a new value
	{
		...
		///////////////////////////////////////////////////LEARNING/////////////////////////////////////////////////////////////////////////
		//char timenext[128];
		//gl_printtime(*next_clear, timenext, 127);
		//trainer.setNextAuctionTime(timenext);
