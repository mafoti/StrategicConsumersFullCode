...
house_e::house_e(MODULE *mod) : residential_enduse(mod)
{
	...
	if (oclass==NULL)  
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_bool, "willWork", PADDR(willWork),
...
int house_e::init(OBJECT *parent)
{
	...
	if (heating_setpoint==0.0)	heating_setpoint = 68.0;
	if (cooling_setpoint==0.0)	cooling_setpoint = 75.0;
	if (design_cooling_setpoint==0.0) design_cooling_setpoint = 75.0;
	if (design_heating_setpoint==0.0) design_heating_setpoint = 68.0;
...
{
	bool istherighthouse = false;
	
	char namebuf[128];
	gl_name(OBJECTHDR(this), namebuf, 127);
	string testcase = "house1";
	if (testcase.compare(namebuf) == 0){
		//printf("inside sync_thermostat of house1\n");
		istherighthouse = true;
		/*if (!willWork){
			aux_heat_deadband = 35;
			thermostat_deadband = 10;
			this->re_override = OV_ON;
		...
		//printf("\n\n\n aux_heat_deadband=%f, dlc_offset=%f, this->re_override=%d \n\n\n", aux_heat_deadband, dlc_offset, (int)this->re_override);
	}
	...
	if (thermostat_control==TC_FULL)
	{
		...
		TauxOn = TheatOn-aux_heat_deadband;
		//if (istherighthouse){
		//	printf("thermostat control is TC_FULL\n");
		//}
	}
	...
	if (this->re_override == OV_ON){
		//if (istherighthouse){
		//	printf("this->re_override == OV_ON, dlc_offset=%f \n", dlc_offset);
		//}
		TcoolOn = TcoolOff;
		TheatOn = TheatOff;
		//TcoolOn = 80;
...
//		thermostat_last_cycle_time = gl_globalclock - thermostat_cycle_time - 1;
	...
	//if (istherighthouse){
	//	printf("last_system_mode : %d\n", (int)last_system_mode);
	//}
